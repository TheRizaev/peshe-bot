...
async def paraphrase_text(text: str) -> str:
    ...
    if has_reactions:
        ...
        system_prompt = (
            ...
            "\n\nЕсли в последних 3 строках нет эмодзи (из списка: "
            + ', '.join(emogies)
            + "), добавь ровно 3 новых строки реакций в конец, вида:\n"
            "<эмодзи> - <короткий текст>(одобрение)\n"
            "<эмодзи> - <короткий текст>(отрицание)\n"
            "<эмодзи> - <короткий текст>(нейтральность>\n\n"
            "Только из списка: " + ', '.join(emogies) + " причем они все должны быть разными. И над реакцией добавь пустую строку "
        )
